...
<template>
  ...
  >
    ...
    <el-form  ref="formRef" :model="form" label-width="80px" :rules="rules">
      ...
      <el-form-item label="代码地址" prop="git_url">
...
import { addPlatform, editPlatform } from '@/api/platform'
...
const handleConfirm = () => {
      ...
      formRef.value.validate(async (valid) => {
        if (valid) {
      props.dialogTitle === '添加平台信息'
        ? await addPlatform(form.value)
        : await editPlatform(form.value)
          props.dialogTitle === '添加平台信息' ? message.value = 'message.addSuccess' : message.value = 'message.updateSuccess'
          ElMessage({
            message: i18n.global.t(message.value),
            type: 'success'
          })
          // 让父组件 执行方法 请求 后台api 获取数据
          emit('initPlatformList')
          handleClose()
        } else {
...
watch(
  ...
  () => {
    ...
    // console.log(form.value, 'form.value 9 9999999999999999999999999 ')
  },
